...
export class SubmeterVersaoFinalComponent implements OnInit {
  ...
  readonly documentos: DocumentoSlot[] = [
    { tipo: 'ATA_DEFESA', label: 'Ata de defesa', descricao: 'Documento gerado após a defesa.', file: null },
    { tipo: 'FOLHA_APROVACAO', label: 'Folha de aprovação assinada', descricao: 'Assinada pela banca.', file: null },
    { tipo: 'FORMULARIOS', label: 'Formulários', descricao: 'Formulários institucionais exigidos.', file: null },
    { tipo: 'DECLARACOES', label: 'Declarações', descricao: 'Declarações obrigatórias.', file: null },
  ];
  ...
  get documentosPendentes(): DocumentoSlot[] {
    return this.documentos.filter((d) => !d.file);
  }
  ...
  submeter(): void {
    ...
    this.documentos.forEach((doc) => {
      if (doc.file) {
        formData.append(`documento_${doc.tipo.toLowerCase()}`, doc.file);
      }
    });
