...
export class LogsSistemaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.submissaoService
      ...
      .subscribe({
        ...
        error: (error: unknown) => {
          this.errorMessage = getApiErrorMessage(error, 'Nao foi possivel carregar os logs do sistema.');
        },
      });
  }
  ...
  get logsFiltrados(): LogAtividade[] {
    return this.logs.filter((l) => {
      if (this.filtroUsuario && !l.usuario_nome.toLowerCase().includes(this.filtroUsuario.toLowerCase())) {
        return false;
      }
      if (this.filtroAcao && l.acao !== this.filtroAcao) {
        return false;
      }
      return true;
    });
  ...
  get acoesDisponiveis(): string[] {
    return [...new Set(this.logs.map((l) => l.acao))].sort();
  }
